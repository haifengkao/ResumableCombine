...
final class FlatMapTests: XCTestCase {
    ...
    func testFlatMapOneByOne() {
        let sequence = Publishers.Sequence<Range<Int>, Never>(sequence: 1 ..< 5)
            .rm.flatMap(maxPublishers: .max(2)) { i in
                Publishers.Sequence(sequence: i ..< i + 4)
            }
        ...
        XCTAssertEqual(tracking.history, [.subscription("FlatMapFix"),
                                          .value(1),
                                          .value(2),
                                          .value(3),
                                          .value(4),
    ...
    func testSendsSubscriptionDownstreamBeforeSubscribingToUpstream() {
        ...
        let flatMap = upstream.rm.flatMap(maxPublishers: .none) { _ in
    ...
    func testChildSubscribeDeadlock() {
        ...
        let flatMap = upstreamPublisher.rm.flatMap(maxPublishers: .max(2)) { Just($0) }
    ...
    func testCrashesWhenRequestedZeroDemand() {
        ...
        let helper = OperatorTestHelper(
            ...
            createSut: { $0.rm.flatMap { _ in CustomPublisher(subscription: nil) } }
        )
    ...
    func testUpstreamDemandWithNoMaxPublishers() {
        ...
        let flatMap = upstreamPublisher.rm.flatMap { Just($0) }
    ...
    func testOuterLockReentrance() {
        ...
        XCTAssertEqual(helper.tracking.history, [.subscription("FlatMapFix")])
    ...
    func testFailsIfUpstreamFails() {
        let helper = OperatorTestHelper(
            ...
            createSut: { $0.rm.flatMap(maxPublishers: .max(3)) { $0 } }
        )
    ...
    func testAllSubscriptionsReleasedOnUpstreamFailure() {
        ...
        let flatMap = upstreamPublisher.rm.flatMap { _ in child }
    ...
    func testAllSubscriptionsReleasedOnChildFailure() {
        ...
        let flatMap = upstreamPublisher.rm.flatMap { children[$0] }
    ...
    func testSendsValuesUntilBufferIsEmpty() throws {
        ...
        XCTAssertEqual(tracking.history, [.subscription("FlatMapFix"), .value(2)])
    ...
    func testFlatMapReceiveSubscriptionTwice() throws {
        let helper = OperatorTestHelper(
            ...
            initialDemand: nil,
            receiveValueDemand: .none,
            createSut: { $0.rm.flatMap(ResultPublisher.init) }
        )
    ...
    func testFlatMapReceiveValueBeforeSubscription() {
        testReceiveValueBeforeSubscription(
            value: 0,
            expected: .history([.subscription("FlatMapFix")], demand: .none),
            { $0.rm.flatMap { _ in Just(0) } }
        )
    }
    ...
    func testFlatMapReceiveCompletionBeforeSubscription() {
        testReceiveCompletionBeforeSubscription(
            inputType: Int.self,
            expected: .history([.subscription("FlatMapFix"), .completion(.finished)]),
            { $0.rm.flatMap { _ in Just(0) } }
        )
    }
    ...
    func testFlatMapReflection() throws {
        ...
        let flatMap = outerPublisher.rm.flatMap { _ in innerPublisher }
        let tracking = TrackingSubscriber()
        flatMap.subscribe(tracking)
